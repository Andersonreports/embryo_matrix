...
async function loadWhoAmI(){
  ...
  if(currentUser.username)renderSessionUser(currentUser.username,currentUser.role);
}
...
function filteredCases(){const keys=['embryologist','client','test','region','result'],q=$('#patientSearch').value.toLowerCase(),month=$('#samplesMonthFilter')?.value||'',storage=$('#storageFilter')?.value||'',transfer=$('#transferFilter')?.value||'',report=$('#reportStatusFilter')?.value||'';return cases.filter(c=>(!q||(c.patient+' '+c.id).toLowerCase().includes(q))&&keys.every(k=>!$(`#${k}Filter`).value||c[k]===$(`#${k}Filter`).value)&&caseMatchesMonth(c,month)&&(!storage||c.embryos?.some(e=>storageInfo(e).status===storage))&&(!transfer||c.embryos?.some(e=>transferInfo(e).status===transfer))&&(!report||(report==='prep'?c.embryos?.some(e=>isReportPrep(e)):c.embryos?.some(e=>reportStatus(e)===report))))}
function embryoDataset(){const month=$('#samplesMonthFilter')?.value||'';const rows=filteredCases().flatMap(c=>(c.embryos?.length?c.embryos.filter(e=>!month||recordMonth(e)===month):[{}]).map((e,i)=>({case:c,embryo:e,identity:c.embryos?.length?resultIdentity(e):{embryo:`Sample ${i+1}`},outcome:e._outcome||c.result})));
  const expanded=registryViewMode==='embryo'?rows.flatMap(x=>{const variants=expandEmbryoRow(x.embryo);if(variants.length<=1)return[x];const numKey=Object.keys(x.embryo).find(k=>k.includes('number of embryos'));return variants.map(v=>({...x,embryo:numKey?{...v,[numKey]:'1'}:v}))}):rows;
  return expanded.sort((a,b)=>(a.embryo._seq??0)-(b.embryo._seq??0))}
const MIN_COLUMN_FILL_RATE=0.10,ALWAYS_SHOW_COLUMNS=new Set(['hospital clinic name']),PINNED_COLUMNS=['dna conc','embryo grade','pgt result','transferred','transfer details'];
function embryoColumns(rows){const seen=new Set(),cols=[];rows.forEach(x=>Object.keys(x.embryo||{}).forEach(k=>{if(k.startsWith('_')||seen.has(k))return;seen.add(k);cols.push(k)}));PINNED_COLUMNS.forEach(k=>{if(!seen.has(k)){seen.add(k);cols.push(k)}});const total=rows.length||1;let filtered=cols.filter(k=>{if(PINNED_COLUMNS.includes(k))return true;const filled=rows.reduce((n,x)=>n+(String(x.embryo?.[k]??'').trim()!==''?1:0),0);return filled>0&&(ALWAYS_SHOW_COLUMNS.has(k)||/dna.*conc|conc.*dna|pgt result|embryo grade|wga conc/.test(k)||filled/total>=MIN_COLUMN_FILL_RATE)});const reorder=(afterPattern,keyPattern)=>{const afterKey=filtered.find(k=>afterPattern.test(k)),movedKey=filtered.find(k=>keyPattern.test(k));if(afterKey&&movedKey&&afterKey!==movedKey){filtered=filtered.filter(k=>k!==movedKey);filtered.splice(filtered.indexOf(afterKey)+1,0,movedKey)}};reorder(/embryo name|embryo id/,/dna.*conc|conc.*dna/);reorder(/dna.*conc|conc.*dna/,/^embryo grade$/);reorder(/seq platform/,/pgt result/);reorder(/^transferred$/,/^transfer details$/);return filtered}
...
function downloadFile(content,type,extension){const blob=new Blob([content],{type}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`EmbryoMatrix-registry-report-${new Date().toISOString().slice(0,10)}.${extension}`;a.click();setTimeout(()=>URL.revokeObjectURL(url),500)}
function downloadRegistry(format){if(format==='pdf'){window.print();return}const{headers,rows}=exportDataset();if(format==='json'){const data=rows.map(row=>Object.fromEntries(headers.map((h,i)=>[h,row[i]??''])));downloadFile(JSON.stringify({exportedAt:new Date().toISOString(),records:data},null,2),'application/json;charset=utf-8','json')}else if(format==='excel'){const cellStyle=` style="mso-number-format:'\\@';"`,table=`<html xmlns:x="urn:schemas-microsoft-com:office:excel"><head><meta charset="UTF-8"><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head><body><table border="1"><colgroup>${headers.map(()=>'<col style="width:130px">').join('')}</colgroup><thead><tr>${headers.map(h=>`<th${cellStyle}>${escapeHtml(h)}</th>`).join('')}</tr></thead><tbody>${rows.map(row=>`<tr>${row.map(v=>`<td${cellStyle}>${escapeHtml(String(v??''))}</td>`).join('')}</tr>`).join('')}</tbody></table></body></html>`;downloadFile('﻿'+table,'application/vnd.ms-excel','xls')}else{const csv='﻿'+[headers,...rows].map(r=>r.map(csvCell).join(',')).join('\r\n');downloadFile(csv,'text/csv;charset=utf-8','csv')}toast(`${rows.length} records exported as ${format==='excel'?'Excel':format.toUpperCase()}`)}
...
function renderVerticalBarChart(id,entries,{color='#0a7180',gradientTo='#20aaa6',height=290,scroll=false,minSlot=44,total:grandTotal=null,of='samples'}={}){
...
function renderTestParetoChart(id,counts,limit=8){
 ...
 const hits=items.map((d,i)=>{const sx=padL+slot*i;return `<rect class="ch-hit" data-col="${i}" x="${sx.toFixed(1)}" y="0" width="${slot.toFixed(1)}" height="${height}" fill="transparent" data-x="${(sx+slot/2).toFixed(1)}" data-y="${Math.min(yFor(d.value),pts[i].y).toFixed(1)}" data-tip="${chartTipAttr({t:d.key,r:[[d.isOther?OTHER_COLOR:BAR_COLOR,'Samples',d.value.toLocaleString()],[LINE_COLOR,'Cumulative',`${cum[i].toFixed(1)}%`]],n:`${(d.value/grandTotal*100).toFixed(1)}% of all samples`})}"/>`}).join('');
 const baseline=`<line x1="${padL}" y1="${(padT+innerH).toFixed(1)}" x2="${w-padR}" y2="${(padT+innerH).toFixed(1)}" stroke="#c9d3ce" stroke-width="1"/>`;
 const legend=`<div style="display:flex;gap:16px;font-size:11px;color:#52625a;margin-bottom:6px"><span style="display:inline-flex;align-items:center;gap:6px"><i style="width:10px;height:10px;border-radius:2px;background:${BAR_COLOR};display:inline-block"></i>Sample count</span><span style="display:inline-flex;align-items:center;gap:6px"><i style="width:14px;height:2px;background:${LINE_COLOR};display:inline-block"></i>Cumulative % of total</span></div>`;
 el.classList.toggle('vbar-scroll',scroll);
...
function allClientsMarkup(){return `<div class="ac-kpis" id="acKpis"></div><div class="ac-top3" id="acTop3"></div><section class="ac-panel"><div class="ac-toolbar"><div class="ac-search"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg><input id="acSearch" type="search" placeholder="Search clients…" aria-label="Search clients"></div><div class="ac-total" id="acTotal" aria-live="polite"></div><div class="prep-segments" id="acSort" role="tablist"><button type="button" class="prep-seg active" data-sort="desc">Most samples</button><button type="button" class="prep-seg" data-sort="asc">Fewest</button><button type="button" class="prep-seg" data-sort="az">A–Z</button></div><select id="acMonth" class="chart-filter" aria-label="Filter clients by month"><option value="">All months</option></select></div><div class="ac-head" aria-hidden="true"><span>#</span><span>Client</span><span>Volume</span><span>Samples</span></div><div class="ac-list" id="acList"></div><div class="ac-foot" id="acFoot"></div></section>`}
function setupAllClientsView(){
 const monthSel=$('#acMonth'),search=$('#acSearch'),sortEl=$('#acSort'),list=$('#acList');
 [...new Set(allEmbryos().map(recordMonth).filter(isDateMonth))].sort().forEach(v=>monthSel.add(new Option(monthLabel(v),v)));
 let sort='desc',entries=[],rankOf=new Map();
 const tile=(label,value,sub='')=>`<div class="ac-kpi"><small>${label}</small><strong>${value}</strong>${sub?`<span>${sub}</span>`:''}</div>`;
 const load=()=>{const counts=countBy(monthFiltered(allEmbryos(),monthSel.value),e=>e._case.client);entries=Object.entries(counts).sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0]));rankOf=new Map(entries.map(([k],i)=>[k,i+1]));
  const n=entries.length,total=entries.reduce((a,[,v])=>a+v,0),big=entries.filter(([,v])=>v>=CLIENT_MIN_SAMPLES).length,single=entries.filter(([,v])=>v===1).length;
  ...
  $('#acTop3').innerHTML=entries.slice(0,3).map(([k,v],i)=>`<button type="button" class="ac-top-card" data-name="${escapeHtml(k)}"><span class="ac-medal m${i+1}">${i+1}</span><span class="ac-top-name" title="${escapeHtml(k)}">${escapeHtml(k)}</span><strong>${v.toLocaleString()}<small> samples</small></strong><span class="ac-top-bar"><span style="width:${(v/peak*100).toFixed(1)}%"></span></span></button>`).join('');
  draw()};
 const draw=()=>{const q=search.value.trim().toLowerCase(),peak=entries[0]?.[1]||1;let rows=entries.filter(([k])=>!q||k.toLowerCase().includes(q));
  if(sort==='asc')rows=[...rows].sort((a,b)=>a[1]-b[1]||a[0].localeCompare(b[0]));else if(sort==='az')rows=[...rows].sort((a,b)=>a[0].localeCompare(b[0]));
  list.innerHTML=rows.length?rows.map(([k,v])=>{const r=rankOf.get(k);return `<div class="ac-row${r<=3?' top':''}" data-name="${escapeHtml(k)}" tabindex="0" role="button"><span class="ac-rank">${r}</span><span class="ac-name" title="${escapeHtml(k)}">${escapeHtml(k)}</span><span class="ac-bar"><span style="width:${Math.max(1.5,v/peak*100).toFixed(1)}%"></span></span><span class="ac-count">${v.toLocaleString()}</span></div>`}).join(''):'<div class="chart-empty">No clients match your search.</div>';
  $('#acTotal').innerHTML=`<small>Total clients</small><strong>${(q?rows.length:entries.length).toLocaleString()}</strong>${q?`<span>of ${entries.length.toLocaleString()}</span>`:''}`;$('#acFoot').textContent=q?`Showing ${rows.length.toLocaleString()} of ${entries.length.toLocaleString()} clients`:`${entries.length.toLocaleString()} clients`};
 ...
 const onPick=e=>{const it=e.target.closest('[data-name]');if(it)openClient(it.dataset.name)};
 list.onclick=onPick;$('#acTop3').onclick=onPick;list.onkeydown=e=>{if(e.key==='Enter')onPick(e)};
...
const QC_FAIL_SIGNALS=['inconclusive','low dna concentration','low reads','no dna detected'];
function qcVerdict(e){const text=`${field(e,['conclusion'])} ${field(e,['result'])}`.toLowerCase().trim();if(text){return QC_FAIL_SIGNALS.some(s=>text.includes(s))?'FAIL':'PASS'}const qc=field(e,['qc']).toUpperCase();return qc==='FAIL'||qc==='PASS'?qc:null}
...
function renderEmbryologistBarChart(id,stats,limit=10){
 ...
 if(!shown.length){el.innerHTML='<div class="chart-empty">No samples in this period.</div>';return}
...
function renderTestBarList(id,counts,total){
 ...
 el.innerHTML=`<div class="tw-list" style="--tw-rows:${Math.ceil(entries.length/2)}">${entries.map(([k,v],i)=>`<button type="button" class="tw-row" data-test="${escapeHtml(k)}" title="${escapeHtml(k)}: ${v.toLocaleString()} samples (${pct(v).toFixed(1)}%)"><span class="tw-name">${escapeHtml(k)}</span><span class="tw-bar"><span style="width:${Math.max(1.5,v/peak*100).toFixed(1)}%"></span></span><strong class="tw-count">${v.toLocaleString()}</strong><span class="tw-pct">${pct(v)<0.1&&v?'<0.1':pct(v).toFixed(1)}%</span></button>`).join('')}</div>`;
...
function embryoTableRows(c,list=resolvedEmbryos(c)){if(!list.length)return Array.from({length:c.samples},(_,i)=>`<tr class="embryo-row"><td><strong>${escapeHtml(c.id)}-S0${i+1}</strong></td><td>—</td><td>—</td><td>—</td><td><span class="result-pill ${i?'normal':c.result.toLowerCase()}">${i?'Normal':c.result}</span></td></tr>`).join('');return list.map(r=>{const identity=resultIdentity(r),out=r._outcome||clinicalOutcome(r),s=storageInfo(r),t=transferInfo(r),location=[s.freezer&&`Freezer ${s.freezer}`,s.rack&&`Rack ${s.rack}`,s.box&&`Box ${s.box}`,s.position&&`Position ${s.position}`].filter(Boolean).join(' · '),name=identity.patient?`${identity.patient}-${identity.embryo}`:(identity.embryo||field(r,['embryo name'])||field(r,['sample name'])),gender=field(r,['gender'])||'—',mapd=field(r,['mapd'])||'—',conclusion=field(r,['conclusion']),storageDetail=[s.barcode,location||'Location not recorded',s.date?`Stored ${s.date}`:'Stored date not recorded',s.notes].filter(Boolean).join(' · '),transferDetail=[t.purpose&&t.purpose!=='Not transferred'&&t.purpose!==t.status?t.purpose:null,t.details||null,t.department?`To ${t.department}`:null,t.sentDate?`Sent ${t.sentDate}`:null,t.sentBy?`By ${t.sentBy}`:null,t.receivedBy?`Received by ${t.receivedBy}`:null,t.receivedDate?`on ${t.receivedDate}`:null,t.returnDate?`Returned ${t.returnDate}`:null,t.remarks].filter(Boolean).join(' · ')||'No transfer details recorded';return `<tr class="embryo-row"><td><strong>${escapeHtml(name)}</strong></td><td>${escapeHtml(field(r,['qc'])||'—')}</td><td>${escapeHtml(field(r,['result'])||'—')}</td><td>${escapeHtml(field(r,['karyotype mwf','karyotype normal wf'])||'—')}</td><td><span class="result-pill ${out.toLowerCase()}">${out}</span></td></tr><tr class="embryo-subrow"><td colspan="5"><p class="${conclusion?'':'result-conclusion'}">${escapeHtml(conclusion||'No conclusion provided')}</p><div class="embryo-mini-meta"><span>Gender ${escapeHtml(gender)}</span><span>MAPD ${escapeHtml(mapd)}</span></div><div class="embryo-trace-line"><span class="mini-badge">${escapeHtml(s.status)}</span>${escapeHtml(storageDetail)}</div><div class="embryo-trace-line"><span class="mini-badge${t.status==='Transferred'?' mini-badge-ok':''}">${escapeHtml(t.status)}</span>${escapeHtml(transferDetail)}</div></td></tr>`}).join('')}
...
function showView(view){
 ...
 $('#headerBackBtn').classList.toggle('hidden',view==='overview'||view==='cases'||view==='samples');
...
$('#newExperiment').onclick=()=>$('#experimentDialog').showModal();$('#headerBackBtn').onclick=()=>showView('overview');$('#quickNote')?.addEventListener('click',()=>$('#noteDialog').showModal());$('.menu-button').onclick=()=>$('.topbar').classList.toggle('open');document.addEventListener('click',()=>{$('#filtersPanel')?.classList.add('hidden');$('#filtersToggle')?.classList.remove('active');$('#exportMenu')?.classList.add('hidden');fitTableHeight($('#caseTable .embryo-columns-table'))});
...
const ACTIVITY_LABELS={login:'Signed in',logout:'Signed out',result_upload:'Result file upload',image_upload:'Embryo image upload',image_delete:'Embryo image deleted',protocol_upload:'Protocol upload',protocol_delete:'Protocol deleted',sheet_sync:'Sheet sync',log_reset:'Log reset'};
const ACTIVITY_TONES={login:'tone-in',logout:'tone-out',result_upload:'tone-result',image_upload:'tone-image',image_delete:'tone-delete',protocol_upload:'tone-protocol',protocol_delete:'tone-delete',sheet_sync:'tone-sync',log_reset:'tone-delete'};
...
function expandEmbryoRow(r){const key=fieldKey(r,['sample name','embryo name','embryo']);if(!key)return[r];const clean=String(r[key]||'').replace(/_L\d+$/i,'').trim(),cut=clean.lastIndexOf('-');if(cut<=0)return[r];const list=clean.slice(cut+1);if(!list.includes(','))return[r];const nums=list.split(',').map(s=>s.trim()).filter(Boolean);if(nums.length<2)return[r];const prefix=clean.slice(0,cut+1);return nums.map(n=>({...r,[key]:`${prefix}${n}`}))}
...
async function handleResultAttach(files){
  ...
  for(const f of files){
    ...
    }catch(e){toast(`${f.name} could not be read`)}
  }
  const resultRows=summaryRows.filter(r=>field(r,['sample name'])&&(field(r,['result'])||field(r,['qc'])));
  if(!resultRows.length){status.textContent='No result rows with a Sample Name and QC/Result value were found.';return}
  ...
  resultRows.forEach(r=>{
    ...
    if(!match){unverified++;return}
    const inconclusiveDetails=inconclusiveIndex.get(`${cleanId(id.patient)}|${id.embryo}`);
    const key=`${match.patientClean}|${id.embryo}|${cleanId(match.sampleId)}`;
    resultByKey.set(key,{...r,...inconclusiveDetails,_computedResult:computeEmbryoResult(r,!!inconclusiveDetails)});
  });
  const allRows=(await kvGet('embryomatrix-imported-cases'))||[];
  let matchedRows=0;
  const mergedAt=new Date().toISOString();
  const updated=allRows.map(row=>{
    const sampleId=field(row,['sample id']),patient=field(row,['patient name','patient']);
    if(!sampleId||!patient)return row;
    const sid=cleanId(sampleId),pat=cleanId(patient);
    const matched=expandEmbryoTags(field(row,['embryo name','embryo id','embryo'])).map(tag=>resultByKey.get(`${pat}|${tag}|${sid}`)).filter(Boolean);
    if(!matched.length)return row;
    matchedRows++;
    const collect=names=>[...new Set(matched.map(r=>field(r,names)).filter(Boolean))].join(', ');
    const patch={_resultMergedAt:mergedAt};
    const resultVal=[...new Set(matched.map(r=>r._computedResult).filter(Boolean))].join(', ');
    if(resultVal)patch['pgt result']=resultVal;
    RESULT_MERGE_FIELDS.forEach(f=>{const v=collect([f]);if(v)patch[f]=v});
    return {...row,...patch};
  });
  await kvSet('embryomatrix-imported-cases',updated);
  await setupCases();
  status.textContent=`${resultRows.length} result row(s) read · ${matchedRows} sample row(s) updated with QC/result data.`+(unverified?` · ${unverified} row(s) skipped (no confirmed Anderson ID match in the Details tab).`:'');
  toast(`${matchedRows} sample(s) updated with QC/results`);
  try{const r=await fetch('/api/upload-log',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({value:{files:files.map(f=>f.name),matched:matchedRows,at:mergedAt}})});if(r.ok)uploadLogCache=(await r.json()).value||uploadLogCache}catch(e){}
  renderUploadLog();
}
async function setupImporter(){let pendingFiles=[];const btn=$('#resultUploadBtn'),status=$('#resultAttachStatus');uploadLogCache=(await kvGet('embryomatrix-upload-log'))||[];renderUploadLog();const resetBtn=$('#uploadLogReset');if(resetBtn)resetBtn.onclick=async()=>{if(!confirm('Clear the result upload log? This cannot be undone.'))return;const r=await fetch('/api/upload-log',{method:'DELETE'}).catch(()=>null);if(r&&r.ok){uploadLogCache=[];renderUploadLog();toast('Upload log cleared')}else toast('Could not reset the log')};$('#resultAttachFile').onchange=e=>{pendingFiles=[...e.target.files];status.textContent=pendingFiles.length?`${pendingFiles.length} file(s) selected: ${pendingFiles.map(f=>f.name).join(', ')}`:'No files selected';btn.disabled=!pendingFiles.length};btn.onclick=async()=>{if(!pendingFiles.length)return;btn.disabled=true;await handleResultAttach(pendingFiles);pendingFiles=[];$('#resultAttachFile').value=''}}
