...
function rebiopsyMarkup(){return `<div class="prep-toolbar rb-toolbar"><div class="prep-segments" id="rbViewToggle" role="tablist" aria-label="Row grouping"><button type="button" class="prep-seg active" data-mode="patient" role="tab" aria-selected="true">Patient view</button><button type="button" class="prep-seg" data-mode="embryo" role="tab" aria-selected="false">Embryo view</button></div><div class="rb-actions"><div class="search-wrap"><span>⌕</span><input id="rebiopsySearch" type="search" placeholder="Search patient, sample ID, test, client…" aria-label="Search re-biopsy samples"></div><button class="toolbar-btn" id="rbSortToggle"><span>⇅</span> <span id="rbSortLabel">Sheet order</span></button><button class="toolbar-btn" id="rbFreezeToggle" title="Keep S.No, Patient and Sample ID in view while scrolling sideways"><span>⇤</span> <span>Freeze columns</span></button><button class="toolbar-btn" id="rbFiltersToggle"><span>▽</span> Filters <b class="filters-badge" id="rbFiltersBadge">0</b></button><div class="export-wrap"><button class="secondary compact" id="rbExport">↓ Export data</button><div class="export-menu hidden" id="rbExportMenu"><button data-format="csv">CSV</button><button data-format="excel">Excel</button><button data-format="json">JSON</button><button data-format="pdf">PDF</button></div></div></div></div><div class="filters-inline rb-filters hidden" id="rbFiltersPanel"><label class="filter-field"><span>Month</span><select data-rb-filter="month"><option value="">All months</option></select></label><label class="filter-field"><span>Test</span><select data-rb-filter="test"><option value="">All tests</option></select></label><label class="filter-field"><span>Client</span><select data-rb-filter="client"><option value="">All clients</option></select></label><label class="filter-field"><span>Embryologist</span><select data-rb-filter="embryologist"><option value="">All embryologists</option></select></label><button class="text-button filters-clear" id="rbClearFilters">Clear all</button></div><div class="prep-table-wrap" id="rebiopsyTableWrap"><div class="chart-empty">Loading…</div></div>`}
function setupRebiopsyView(){
 const records=allEmbryos().filter(isRebiopsy).map(e=>({e,c:e._case,patient:e._case.patient||field(e,['patient name'])||'—',sampleId:field(e,['sample id'])||e._case.id||'—',embryos:field(e,['embryo name'])||'—',count:embryoUnits(e),test:canonicalTestName(field(e,['test name']))||e._case.test||'—',client:e._case.client||'—',embryologist:e._case.embryologist||'—',received:field(e,['date sample received'])||'—',remarks:field(e,['remarks'])||'—',month:recordMonth(e)}));
 // Patient view: one row per patient case, its re-biopsied records merged.
 const byPatient=()=>{const m=new Map();for(const r of records){let g=m.get(r.c);if(!g)m.set(r.c,g={...r,sampleIds:new Set(),embryoList:[],remarkSet:new Set(),count:0});g.sampleIds.add(r.sampleId);g.embryoList.push(r.embryos);g.remarkSet.add(r.remarks);g.count+=r.count}return[...m.values()].map(g=>({...g,sampleId:[...g.sampleIds].join(', '),embryos:g.embryoList.join(', '),remarks:[...g.remarkSet].join(' | ')}))};
 let q='',mode='patient',reversed=false,frozen=false;const filters={month:'',test:'',client:'',embryologist:''};
 const wrap=$('#rebiopsyTableWrap'),search=$('#rebiopsySearch'),panel=$('#rbFiltersPanel');
 panel.querySelectorAll('[data-rb-filter]').forEach(sel=>{const k=sel.dataset.rbFilter,vals=[...new Set(records.map(r=>r[k]).filter(v=>v&&v!=='—'&&(k!=='month'||isDateMonth(v))))].sort();vals.forEach(v=>sel.add(new Option(k==='month'?monthLabel(v):v,v)));if(k==='month')[...sel.options].slice(1).reverse().forEach(o=>sel.appendChild(o))});
 const COLS=[['S.No','num'],['Patient'],['Sample ID'],['Embryos'],['Test'],['Client'],['Embryologist'],['Received'],['Remarks']];
 const current=()=>{const needle=q.trim().toLowerCase();let list=(mode==='patient'?byPatient():records).filter(r=>Object.entries(filters).every(([k,v])=>!v||r[k]===v)&&(!needle||[r.patient,r.sampleId,r.client,r.embryologist,r.test,r.embryos,r.remarks].join(' ').toLowerCase().includes(needle)));return reversed?list.reverse():list};
 const cells=(r,i)=>[i+1,r.patient,r.sampleId,`${r.embryos} (${r.count})`,r.test,r.client,r.embryologist,r.received,r.remarks];
 const updateBadge=()=>{const n=Object.values(filters).filter(Boolean).length,b=$('#rbFiltersBadge');b.textContent=n;b.classList.toggle('has-active',n>0)};
 const draw=()=>{const list=current();updateBadge();$('#rbFreezeToggle').classList.toggle('active',frozen);
  if(!list.length){wrap.innerHTML='<div class="chart-empty">No re-biopsy samples match.</div>';return}
  wrap.innerHTML=`<table class="prep-table"><thead><tr>${COLS.map(([h,c])=>`<th${c?` class="${c}"`:''}>${h}</th>`).join('')}</tr></thead><tbody>${list.map((r,i)=>`<tr data-i="${i}" tabindex="0"><td class="num muted">${i+1}</td><td class="strong">${escapeHtml(r.patient)}</td><td class="mono">${escapeHtml(r.sampleId)}</td><td title="${escapeHtml(r.embryos)}">${escapeHtml(r.embryos)} <small>(${r.count})</small></td><td>${escapeHtml(r.test)}</td><td class="clip" title="${escapeHtml(r.client)}">${escapeHtml(r.client)}</td><td class="clip" title="${escapeHtml(r.embryologist)}">${escapeHtml(r.embryologist)}</td><td class="muted">${escapeHtml(r.received)}</td><td class="clip" title="${escapeHtml(r.remarks)}">${escapeHtml(r.remarks)}</td></tr>`).join('')}</tbody></table>`;
  wrap._list=list;applyColumnFreeze(wrap,frozen?3:0)};
 search.oninput=()=>{q=search.value;draw()};
 $('#rbViewToggle').onclick=e=>{const b=e.target.closest('[data-mode]');if(!b)return;mode=b.dataset.mode;$('#rbViewToggle').querySelectorAll('.prep-seg').forEach(x=>{x.classList.toggle('active',x===b);x.setAttribute('aria-selected',x===b)});draw()};
 $('#rbSortToggle').onclick=()=>{reversed=!reversed;$('#rbSortLabel').textContent=reversed?'Reversed':'Sheet order';$('#rbSortToggle').classList.toggle('active',reversed);draw()};
 $('#rbFreezeToggle').onclick=()=>{frozen=!frozen;draw()};
 $('#rbFiltersToggle').onclick=()=>{panel.classList.toggle('hidden');$('#rbFiltersToggle').classList.toggle('active',!panel.classList.contains('hidden'))};
 panel.onchange=e=>{const s=e.target.closest('[data-rb-filter]');if(s){filters[s.dataset.rbFilter]=s.value;draw()}};
 $('#rbClearFilters').onclick=()=>{Object.keys(filters).forEach(k=>filters[k]='');panel.querySelectorAll('select').forEach(s=>s.value='');draw()};
 const menu=$('#rbExportMenu');
 $('#rbExport').onclick=e=>{e.stopPropagation();menu.classList.toggle('hidden')};
 menu.onclick=e=>{const b=e.target.closest('[data-format]');if(!b)return;menu.classList.add('hidden');const list=current();downloadRegistry(b.dataset.format,{headers:COLS.map(([h])=>h),rows:list.map(cells)},'rebiopsy')};
 document.addEventListener('click',e=>{if(!e.target.closest('#rbExport'))menu.classList.add('hidden')});
 const open=tr=>{const r=wrap._list?.[+tr.dataset.i];if(r)openPatient(r.c)};
 wrap.onclick=e=>{const tr=e.target.closest('tbody tr');if(tr)open(tr)};
 wrap.onkeydown=e=>{if(e.key==='Enter'){const tr=e.target.closest('tbody tr');if(tr)open(tr)}};
 draw();
}
...
function overviewRows(){const rows=allEmbryos();if(overviewUnit!=='embryos')return rows;return rows.flatMap(e=>{const split=expandEmbryoRow(e);if(split.length>1)return split;const n=Math.round(embryoUnits(e));return n>1?Array.from({length:n},()=>split[0]):split})}
