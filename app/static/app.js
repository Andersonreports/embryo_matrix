...
function setupRebiopsyView(){
 ...
 let q='',frozen=false;
 const wrap=$('#rebiopsyTableWrap'),search=$('#rebiopsySearch'),monthSel=$('#rebiopsyMonthFilter'),testSel=$('#rebiopsyTestFilter'),clientSel=$('#rebiopsyClientFilter'),embSel=$('#rebiopsyEmbryologistFilter'),badge=$('#rebiopsyFiltersBadge');
 ...
 const draw=()=>{const needle=q.trim().toLowerCase(),list=rows.filter(r=>(!needle||[r.patient,r.sampleId,r.client,r.embryologist,r.test,r.embryos,r.remarks].join(' ').toLowerCase().includes(needle))&&(!monthSel.value||r.month===monthSel.value)&&(!testSel.value||r.test===testSel.value)&&(!clientSel.value||r.client===clientSel.value)&&(!embSel.value||r.embryologist===embSel.value));
  updateBadge();
  ...
  applyColumnFreeze(wrap,frozen?3:0);
  fitTableHeight(wrap)};
 search.oninput=()=>{q=search.value;draw()};
