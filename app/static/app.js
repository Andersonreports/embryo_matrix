...
function filteredCases(){const keys=['embryologist','client','region','result'],test=$('#testFilter')?.value||'',q=$('#patientSearch').value.toLowerCase(),month=$('#samplesMonthFilter')?.value||'',storage=$('#storageFilter')?.value||'',transfer=$('#transferFilter')?.value||'',report=$('#reportStatusFilter')?.value||'';return cases.filter(c=>(!q||(c.patient+' '+c.id).toLowerCase().includes(q))&&keys.every(k=>!$(`#${k}Filter`).value||c[k]===$(`#${k}Filter`).value)&&(!test||(c.embryos?.length?c.embryos.some(e=>rowTestName(e)===test):c.test===test))&&caseMatchesMonth(c,month)&&(!storage||c.embryos?.some(e=>storageInfo(e).status===storage))&&(!transfer||c.embryos?.some(e=>transferInfo(e).status===transfer))&&(!report||(report==='prep'?c.embryos?.some(e=>isReportPrep(e)):c.embryos?.some(e=>reportStatus(e)===report))))}
...
function ownDnaValue(text,embryo){const want=cleanId(embryo),num=(/(\d+)\D*$/.exec(String(embryo))||[])[1];for(const part of String(text||'').split(',')){const m=/^\s*([^:]+):\s*(.+?)\s*$/.exec(part);if(!m)continue;const tag=m[1].trim();if(cleanId(tag)===want||(/^\d+$/.test(tag)&&num&&Number(tag)===Number(num)))return m[2]}return''}
function withOwnDna(e){const name=field(e,['embryo name','embryo id']),out={...e};DNA_FIELDS.forEach(f=>{if(f in out)out[f]=ownDnaValue(out[f],name)});return out}
function embryoDataset(){const month=$('#samplesMonthFilter')?.value||'',test=$('#testFilter')?.value||'';const rows=filteredCases().flatMap(c=>(c.embryos?.length?c.embryos.filter(e=>(!month||recordMonth(e)===month)&&(!test||rowTestName(e)===test)):[{}]).map((e,i)=>({case:c,embryo:e,identity:c.embryos?.length?resultIdentity(e):{embryo:`Sample ${i+1}`},outcome:e._outcome||c.result})));
...
const MIN_COLUMN_FILL_RATE=0.10,ALWAYS_SHOW_COLUMNS=new Set(['hospital clinic name']),PINNED_COLUMNS=['dna conc unpurified','dna conc purified','embryo grade','pgt result','transferred','transfer details'];
function embryoColumns(rows){const seen=new Set(),cols=[];rows.forEach(x=>Object.keys(x.embryo||{}).forEach(k=>{if(k.startsWith('_')||seen.has(k))return;seen.add(k);cols.push(k)}));PINNED_COLUMNS.forEach(k=>{if(!seen.has(k)){seen.add(k);cols.push(k)}});const total=rows.length||1;let filtered=cols.filter(k=>{if(PINNED_COLUMNS.includes(k))return true;const filled=rows.reduce((n,x)=>n+(String(x.embryo?.[k]??'').trim()!==''?1:0),0);return filled>0&&(ALWAYS_SHOW_COLUMNS.has(k)||/dna.*conc|conc.*dna|pgt result|embryo grade|wga conc/.test(k)||filled/total>=MIN_COLUMN_FILL_RATE)});const reorder=(afterPattern,keyPattern)=>{const afterKey=filtered.find(k=>afterPattern.test(k)),movedKey=filtered.find(k=>keyPattern.test(k));if(afterKey&&movedKey&&afterKey!==movedKey){filtered=filtered.filter(k=>k!==movedKey);filtered.splice(filtered.indexOf(afterKey)+1,0,movedKey)}};reorder(/embryo name|embryo id/,/dna conc unpurified/);reorder(/dna conc unpurified/,/dna conc purified/);reorder(/dna conc purified/,/^embryo grade$/);reorder(/^embryo grade$/,/pgt result/);reorder(/^transferred$/,/^transfer details$/);return filtered}
...
function rebiopsyMarkup(){return `<div class="prep-toolbar"><div class="prep-search"><input id="rebiopsySearch" type="search" placeholder="Search patient, sample ID, client, embryologist…" aria-label="Search re-biopsy samples"></div></div><div class="prep-table-wrap" id="rebiopsyTableWrap"><div class="chart-empty">Loading…</div></div>`}
function setupRebiopsyView(){
 ...
 let q='';
 const wrap=$('#rebiopsyTableWrap'),search=$('#rebiopsySearch');
 const draw=()=>{const needle=q.trim().toLowerCase(),list=rows.filter(r=>!needle||[r.patient,r.sampleId,r.client,r.embryologist,r.test,r.embryos,r.remarks].join(' ').toLowerCase().includes(needle));
  if(!list.length){wrap.innerHTML='<div class="chart-empty">No re-biopsy samples match.</div>';return}
  wrap.innerHTML=`<table class="prep-table"><thead><tr><th class="num">S.No</th><th>Patient</th><th>Sample ID</th><th>Embryos</th><th>Test</th><th>Client</th><th>Embryologist</th><th>Received</th><th>Remarks</th></tr></thead><tbody>${list.map((r,i)=>`<tr data-i="${rows.indexOf(r)}" tabindex="0"><td class="num muted">${i+1}</td><td class="strong">${escapeHtml(r.patient)}</td><td class="mono">${escapeHtml(r.sampleId)}</td><td title="${escapeHtml(r.embryos)}">${escapeHtml(r.embryos)}</td><td>${escapeHtml(r.test)}</td><td class="clip" title="${escapeHtml(r.client)}">${escapeHtml(r.client)}</td><td class="clip" title="${escapeHtml(r.embryologist)}">${escapeHtml(r.embryologist)}</td><td class="muted">${escapeHtml(r.received)}</td><td class="clip" title="${escapeHtml(r.remarks)}">${escapeHtml(r.remarks)}</td></tr>`).join('')}</tbody></table>`};
 search.oninput=()=>{q=search.value;draw()};
 const open=tr=>{const r=rows[+tr.dataset.i];if(r)openPatient(r.e._case)};
 wrap.onclick=e=>{const tr=e.target.closest('tbody tr');if(tr)open(tr)};
 wrap.onkeydown=e=>{if(e.key==='Enter'){const tr=e.target.closest('tbody tr');if(tr)open(tr)}};
...
function embryoTableRows(c,list=resolvedEmbryos(c)){if(!list.length)return Array.from({length:c.samples},(_,i)=>`<tr class="embryo-row"><td><strong>${escapeHtml(c.id)}-S0${i+1}</strong></td><td>—</td><td>—</td><td>—</td><td><span class="result-pill ${i?'normal':c.result.toLowerCase()}">${i?'Normal':c.result}</span></td></tr>`).join('');return list.map(r=>{const identity=resultIdentity(r),out=conclusionClass(r)||((field(r,['conclusion'])||field(r,['result'])||field(r,['qc']))?(r._outcome||clinicalOutcome(r)):'N/A'),s=storageInfo(r),t=transferInfo(r),location=[s.freezer&&`Freezer ${s.freezer}`,s.rack&&`Rack ${s.rack}`,s.box&&`Box ${s.box}`,s.position&&`Position ${s.position}`].filter(Boolean).join(' · '),name=identity.patient?`${identity.patient}-${identity.embryo}`:(identity.embryo||field(r,['embryo name'])||field(r,['sample name'])),mapd=field(r,['mapd'])||'—',mtcopy=field(r,['mtcopy'])||'—',uniquereads=field(r,['uniquereads'])||'—',bincv=field(r,['bincv'])||'—',cnvmergecv=field(r,['cnvmergecv'])||'—',cnvpq=field(r,['cnvpq'])||'—',autosomes=field(r,['autosomes'])||'—',conclusion=field(r,['conclusion']),storageDetail=[s.barcode,location||'Location not recorded',s.date?`Stored ${s.date}`:'Stored date not recorded',s.notes].filter(Boolean).join(' · '),transferDetail=[t.purpose&&t.purpose!=='Not transferred'&&t.purpose!==t.status?t.purpose:null,t.details||null,t.department?`To ${t.department}`:null,t.sentDate?`Sent ${t.sentDate}`:null,t.sentBy?`By ${t.sentBy}`:null,t.receivedBy?`Received by ${t.receivedBy}`:null,t.receivedDate?`on ${t.receivedDate}`:null,t.returnDate?`Returned ${t.returnDate}`:null,t.remarks].filter(Boolean).join(' · ')||'No transfer details recorded';return `<tr class="embryo-row"><td><strong>${escapeHtml(name)}</strong></td><td>${escapeHtml(qcVerdict(r)==='FAIL'?'FAIL':(field(r,['qc'])||'—'))}</td><td>${escapeHtml(field(r,['result'])||'—')}</td><td>${escapeHtml(field(r,['karyotype mwf','karyotype normal wf'])||'—')}</td><td><span class="result-pill ${out.toLowerCase().replace(/[^a-z]/g,'')}">${out}</span></td></tr><tr class="embryo-subrow"><td colspan="5"><p class="${conclusion?'':'result-conclusion'}">${escapeHtml(conclusion||'No conclusion provided')}</p><div class="embryo-mini-meta"><span>MAPD ${escapeHtml(mapd)}</span><span>MTcopy ${escapeHtml(mtcopy)}</span><span>Unique reads ${escapeHtml(uniquereads)}</span><span>BinCV ${escapeHtml(bincv)}</span><span>CNVMergeCV ${escapeHtml(cnvmergecv)}</span><span>CNVpq ${escapeHtml(cnvpq)}</span><span>Autosomes ${escapeHtml(autosomes)}</span></div><div class="embryo-trace-line"><span class="mini-badge">${escapeHtml(s.status)}</span>${escapeHtml(storageDetail)}</div><div class="embryo-trace-line"><span class="mini-badge${t.status==='Transferred'?' mini-badge-ok':''}">${escapeHtml(t.status)}</span>${escapeHtml(transferDetail)}</div></td></tr>`}).join('')}
...
function expandEmbryoRow(r){const key=fieldKey(r,['sample name','embryo name','embryo']);if(!key)return[r];const clean=String(r[key]||'').replace(/_L\d+$/i,'').trim(),cut=clean.lastIndexOf('-');if(cut<=0)return[withEmbryoResult(r,clean)];const list=clean.slice(cut+1);if(!list.includes(','))return[withEmbryoResult(r,clean)];const nums=list.split(',').map(s=>s.trim()).filter(Boolean);if(nums.length<2)return[withEmbryoResult(r,clean)];const prefix=clean.slice(0,cut+1);return nums.map(n=>withEmbryoResult({...r,[key]:`${prefix}${n}`},`${prefix}${n}`))}
...
function rowResultKeys(row){const sampleId=field(row,['sample id']),patient=field(row,['patient name','patient']);if(!sampleId||!patient)return[];const sid=cleanId(sampleId),pat=cleanId(patient);return expandEmbryoTags(field(row,['embryo name','embryo id','embryo'])).map(tag=>`${pat}|${tag}|${sid}`)}
...
async function parseResultFile(f){
 ...
 const byKey=new Map;let unverified=0;
 resultRows.forEach(r=>{
  ...
  byKey.set(`${match.patientClean}|${id.embryo}|${cleanId(match.sampleId)}`,{...r,...inconclusiveDetails,_computedResult:computeEmbryoResult(r,!!inconclusiveDetails)});
 });
 return {file:f,byKey,count:resultRows.length,unverified};
}
async function handleResultAttach(files){
 const status=$('#resultAttachStatus');
 status.textContent=`Reading ${files.length} file(s)…`;
 const parsed=[];
 for(const f of files){try{parsed.push(await parseResultFile(f))}catch(e){toast(`${f.name} could not be read`)}}
 const unverified=parsed.reduce((n,p)=>n+p.unverified,0),skippedNote=unverified?` · ${unverified} row(s) skipped (no confirmed Anderson ID match in the Details tab).`:'';
 if(!parsed.some(p=>p.byKey.size)){status.textContent='No result rows with a Sample Name and QC/Result value were found.'+skippedNote;return}
 ...
 for(const e of entries){try{await fetch('/api/result-files',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({value:{id:e.id,fileName:e.file.name,run:runNumberOf(e.file.name),at:mergedAt,samples:e.keys,sampleNames:e.keys.map(k=>field(e.byKey.get(k),['sample name'])),matched:e.keys.length}})})}catch(err){}}
