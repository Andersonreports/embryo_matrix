...
function embryoColumns(rows){const seen=new Set(),cols=[];rows.forEach(x=>Object.keys(x.embryo||{}).forEach(k=>{if(k.startsWith('_')||seen.has(k))return;seen.add(k);cols.push(k)}));PINNED_COLUMNS.forEach(k=>{if(!seen.has(k)){seen.add(k);cols.push(k)}});const total=rows.length||1;let filtered=cols.filter(k=>{if(PINNED_COLUMNS.includes(k))return true;const filled=rows.reduce((n,x)=>n+(String(x.embryo?.[k]??'').trim()!==''?1:0),0);return filled>0&&(ALWAYS_SHOW_COLUMNS.has(k)||/dna.*conc|conc.*dna|pgt result|embryo grade|wga conc/.test(k)||filled/total>=MIN_COLUMN_FILL_RATE)});const reorder=(afterPattern,keyPattern)=>{const afterKey=filtered.find(k=>afterPattern.test(k)),movedKey=filtered.find(k=>keyPattern.test(k));if(afterKey&&movedKey&&afterKey!==movedKey){filtered=filtered.filter(k=>k!==movedKey);filtered.splice(filtered.indexOf(afterKey)+1,0,movedKey)}};reorder(/embryo name|embryo id/,/dna conc unpurified/);reorder(/dna conc unpurified/,/dna conc purified/);reorder(/dna conc purified/,/^embryo grade$/);reorder(/seq platform/,/pgt result/);reorder(/^transferred$/,/^transfer details$/);return filtered}
