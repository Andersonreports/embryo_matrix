...
const COLUMN_LABELS={'dna conc unpurified':'WGA CONC UNPURIFIED','dna conc purified':'WGA CONC PURIFIED'};
...
function withCellEdits(r){
 ...
 const sampleId=field(r,['sample id','sample no','box number']),tags=expandEmbryoTags(field(r,['embryo name','embryo id','embryo']));
 if(!sampleId||!tags.length)return r;
 ...
 for(const col of Object.keys(CELL_EDIT_EXISTING)){
  ...
  if(!out)out={...r};
  out[col]=vals.some(([,v])=>v)?vals.map(([t,v])=>`${t}: ${v||'-'}`).join(', '):'-';
...
function embryoDataset(){const q=$('#patientSearch')?.value.toLowerCase()||'',month=q?'':($('#samplesMonthFilter')?.value||''),test=$('#testFilter')?.value||'',run=$('#runFilter')?.value||'';const rows=filteredCases().flatMap(c=>(c.embryos?.length?c.embryos.filter(e=>(!month||recordMonth(e)===month)&&(!test||rowTestName(e)===test)&&(!run||runsOf(e).includes(run))):[{}]).map((e,i)=>({case:c,embryo:e,identity:c.embryos?.length?resultIdentity(e):{embryo:`Sample ${i+1}`},outcome:e._outcome||c.result})));
...
function renderCases(){const datasetRows=embryoDataset(),rows=sortNewestFirst?[...datasetRows].reverse():datasetRows,empty='<div class="empty-action"><h3>No records available</h3><p>Import the monthly register and embryo result files, or clear the current filters.</p><button class="primary" data-view="import">Upload result</button></div>',naturalCols=embryoColumns(rows),cols=[...naturalCols.filter(k=>frozenColumnKeys.has(k)),...naturalCols.filter(k=>!frozenColumnKeys.has(k))],chipCols=new Set(['sample id','box number','case id']);if($('#registryCount'))$('#registryCount').textContent=rows.length?`· ${rows.length.toLocaleString()} records`:'';const embryoStat=$('#samplesEmbryoStat');if(embryoStat)embryoStat.textContent=rows.reduce((s,x)=>s+embryoRowsOf(x.embryo).length,0).toLocaleString();const caseCountEl=$('#caseCount');if(caseCountEl)caseCountEl.textContent=registryViewMode==='embryo'?allEmbryos().flatMap(expandEmbryoRow).length:allEmbryos().length;$('#caseTable').innerHTML=`<div class="embryo-columns-table"><table><thead><tr>${cols.map(k=>`<th>${escapeHtml(columnLabel(k))}<button class="col-freeze-btn${frozenColumnKeys.has(k)?' pinned':''}" data-freeze-col="${escapeHtml(k)}" title="${frozenColumnKeys.has(k)?'Unfreeze this column':'Freeze this column'}">${PIN_ICON}</button></th>`).join('')}</tr></thead><tbody>${rows.map(x=>{const focus=registryViewMode==='embryo'?embryoDisplayId(x.embryo):'';return `<tr data-case="${escapeHtml(x.case.id)}"${focus?` data-embryo-focus="${escapeHtml(focus)}"`:''}>${cols.map(k=>{const v=String(x.embryo[k]??'');if(chipCols.has(k))return `<td>${v?`<span class="id-chip">${escapeHtml(v)}</span>`:''}</td>`;if(registryViewMode==='embryo'&&EDITABLE_REGISTRY_COLS.has(k)){const sampleId=field(x.embryo,['sample id','sample no','box number']),tags=expandEmbryoTags(field(x.embryo,['embryo name','embryo id','embryo']));if(sampleId&&tags.length===1)return `<td class="editable-cell" contenteditable="true" spellcheck="false" title="Click to edit" data-sample-id="${escapeHtml(sampleId)}" data-embryo="${escapeHtml(tags[0])}" data-column="${escapeHtml(k)}" data-original="${escapeHtml(v)}">${escapeHtml(v)}</td>`}return `<td${v.length>40?` class="wrap-cell" title="${escapeHtml(v)}"`:''}>${escapeHtml(v)}</td>`}).join('')}</tr>`}).join('')}</tbody></table></div>`+(rows.length?'':empty);$$('[data-case]').forEach(r=>r.onclick=()=>openPatient(cases.find(c=>c.id===r.dataset.case),r.dataset.embryoFocus||''));$$('#caseTable [data-view]').forEach(b=>b.onclick=()=>showView(b.dataset.view));$$('#caseTable [data-freeze-col]').forEach(b=>b.onclick=e=>{e.stopPropagation();const k=b.dataset.freezeCol;if(frozenColumnKeys.has(k))frozenColumnKeys.delete(k);else frozenColumnKeys.add(k);renderCases()});applyColumnFreeze($('#caseTable'),frozenColumnKeys.size);const freezeBtn=$('#freezeToggle');if(freezeBtn){freezeBtn.classList.toggle('active',frozenColumnKeys.size>0);$('#freezeToggleLabel').textContent=frozenColumnKeys.size?`Freeze columns (${frozenColumnKeys.size})`:'Freeze columns'}updateFiltersBadge();fitTableHeight($('#caseTable .embryo-columns-table'));wireEditableCells()}
function wireEditableCells(){$$('#caseTable .editable-cell').forEach(td=>{td.onclick=e=>e.stopPropagation();td.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();td.blur()}else if(e.key==='Escape'){td.textContent=td.dataset.original;td.blur()}};td.onblur=async()=>{const val=td.textContent.trim(),orig=td.dataset.original;if(val===orig)return;const{sampleId,embryo,column}=td.dataset;td.classList.add('cell-saving');try{const res=await fetch('/api/cell-edits',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sampleId,embryo,column,value:val})});if(!res.ok)throw new Error('save failed');toast('Edit saved');await setupCases()}catch(err){toast('Could not save edit');td.textContent=orig;td.classList.remove('cell-saving')}}})}
function updateFiltersBadge(){const ids=['samplesMonthFilter','testFilter','runFilter','clientFilter','embryologistFilter','regionFilter','resultFilter','storageFilter','transferFilter','reportStatusFilter'],active=ids.filter(id=>$(`#${id}`)?.value).length,badge=$('#filtersBadge');if(!badge)return;badge.textContent=active;badge.classList.toggle('has-active',active>0)}
...
function fullRegistryDataset(){const rows=cases.flatMap(c=>c.embryos?.length?c.embryos.map((e,i)=>({case:c,embryo:e,identity:resultIdentity(e),outcome:e._outcome||c.result})):[{case:c,embryo:{},identity:{embryo:'Sample 1'},outcome:c.result}]),cols=embryoColumns(rows);return{headers:cols.map(columnLabel),rows:rows.map(x=>cols.map(k=>x.embryo[k]??''))}}
...
<div id="trfNewPane"><div class="trf-formbar"><span class="trf-formbar-note">Fields marked <b class="td-req">*</b> are required. Signatures, clinician seal and Form G are signed on the printed copy.</span><div class="trf-formbar-actions"><button type="button" class="secondary compact" id="trfClear">Clear form</button><button type="button" class="primary compact" id="trfSubmit">Submit TRF</button></div></div><div class="trf-error hidden" id="trfError" role="alert"></div><div class="trf-sheet" id="trfSheet"></div><datalist id="trfClinicList"></datalist></div>
...
async function setupTrfsView(){
 ...
 try{const names=new Set();clientDirectory().forEach(({brand,branches})=>branches.forEach(b=>names.add(b.branch?`${brand} – ${b.branch}`:brand)));$('#trfClinicList').innerHTML=[...names].sort().map(n=>`<option value="${escapeHtml(n)}">`).join('')}catch{}
 ...
 // Submitted list
 const STATUSES=['New','Received','Rejected','All'];
 const fmt=iso=>iso?new Date(iso).toLocaleString([],{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'}):'—';
 const testNames=t=>(t||[]).map(k=>({'PGT-A':'PGT-A','EMBRYO_SURE':'Embryo Sure','PGT-SR':'PGT-SR','PGT-HLA':'HLA C typing'}[k]||k)).join(', ');
 const draw=()=>{const q=search.value.trim().toLowerCase();
  seg.innerHTML=STATUSES.map(s=>`<button type="button" class="prep-seg${s===status?' active':''}" data-status="${s}">${s} <span class="ac-scope-n">${(s==='All'?rows:rows.filter(r=>r.status===s)).length}</span></button>`).join('');
  const list=rows.filter(r=>(status==='All'||r.status===status)&&(!q||[r.ref,r.patient,r.clinic,r.doctor].join(' ').toLowerCase().includes(q)));
 ...
 wrap.onclick=e=>{const tr=e.target.closest('tbody tr');if(tr)open(tr.dataset.id)};
...
function openTrfDialog(t,setStatus){let dlg=$('#trfDialog');
 ...
 dlg.innerHTML=`<div class="trf-dialog-bar"><div><strong>${escapeHtml(t.ref)}</strong> · ${escapeHtml(t.patient)} <span class="trf-status s-${t.status.toLowerCase()}">${escapeHtml(t.status)}</span>${t.statusBy?`<small> by ${escapeHtml(t.statusBy)}</small>`:''}</div><div class="trf-dialog-actions">${t.status!=='Received'?'<button type="button" class="secondary compact" data-set="Received">Mark received</button>':''}${t.status!=='Rejected'?'<button type="button" class="secondary compact" data-set="Rejected">Reject</button>':''}${t.status!=='New'?'<button type="button" class="secondary compact" data-set="New">Move back to New</button>':''}<button type="button" class="primary compact" id="trfPrint">Print / PDF</button><button type="button" class="close" aria-label="Close">×</button></div></div><div class="trf-dialog-doc">${trfPagesHtml(t.data,meta)}</div>`;
...
// A "batch"/"run" is one uploaded result file (embryomatrix-result-files, see cell_edits-style
// overlay pattern) - each embryo it touched carries a matching _fileId in its _embryoResults entry
// (legacy pre-tracking uploads instead share a _resultMergedAt timestamp with no _fileId at all).
function embryosForRun(run){const out=[];cases.forEach(c=>{(c.embryos||[]).forEach(row=>{if(run.legacy){if(row._resultMergedAt!==run.at)return;Object.entries(row._embryoResults||{}).forEach(([tag,data])=>{if(!data._fileId)out.push({case:c,row,tag,data})})}else{Object.entries(row._embryoResults||{}).forEach(([tag,data])=>{if(data._fileId===run.id)out.push({case:c,row,tag,data})})}})});return out}
function runBreakdown(run){const list=embryosForRun(run),cls=list.map(x=>conclusionClass(x.data)),qc=list.map(x=>qcVerdict(x.data));return{total:list.length,euploid:cls.filter(c=>c==='Normal').length,aneuploid:cls.filter(c=>c==='Abnormal').length,mosaicInc:cls.filter(c=>c==='Mosaic'||c==='Inconclusive').length,pass:qc.filter(v=>v==='PASS').length,fail:qc.filter(v=>v==='FAIL').length,list}}
function renderHomeStats(){const rows=allEmbryos(),set=(id,v)=>{const el=$(id);if(el)el.textContent=v.toLocaleString()};set('#homeBatchesStat',resultFilesCache.length);set('#homeEmbryosStat',rows.reduce((s,e)=>s+embryoRowsOf(e).length,0));set('#homeOngoingStat',rows.filter(e=>e._importSource==='Pending').length);set('#homeCompletedStat',rows.filter(e=>reportStatus(e)==='Completed').length)}
function renderRunList(){const list=$('#runList');if(!list)return;const q=($('#runSearch')?.value||'').trim().toLowerCase(),runs=[...resultFilesCache].filter(r=>!q||`${r.run} ${r.fileName}`.toLowerCase().includes(q));if(!runs.length){list.innerHTML=`<div class="chart-empty">${resultFilesCache.length?'No batches match.':'No result files uploaded yet.'}</div>`;return}
 list.innerHTML=runs.map((r,i)=>{const b=runBreakdown(r),pct=n=>b.total?Math.round(n/b.total*100):0;return `<article class="batch-card" data-i="${i}" tabindex="0" role="button"><div class="batch-card-top"><div><strong class="batch-run">${escapeHtml(r.run||'Run')}</strong><small class="batch-file" title="${escapeHtml(r.fileName)}">${escapeHtml(r.fileName)}</small></div><span class="batch-date">${r.at?new Date(r.at).toLocaleDateString():'—'}</span></div><div class="batch-samples"><strong>${b.total.toLocaleString()}</strong>samples</div><div class="batch-bar"><span class="seg-euploid" style="width:${pct(b.euploid)}%"></span><span class="seg-aneuploid" style="width:${pct(b.aneuploid)}%"></span><span class="seg-mosaic" style="width:${pct(b.mosaicInc)}%"></span></div><div class="batch-legend"><span><span class="dot euploid"></span>${b.euploid.toLocaleString()} Euploid</span><span><span class="dot aneuploid"></span>${b.aneuploid.toLocaleString()} Aneuploid</span><span><span class="dot mosaic"></span>${b.mosaicInc.toLocaleString()} Mosaic/Inc.</span></div><div class="batch-qc"><span class="qc-pass">${b.pass.toLocaleString()} QC Pass</span><span class="qc-fail">${b.fail.toLocaleString()} QC Fail</span></div></article>`}).join('');
 list.querySelectorAll('.batch-card').forEach((el,i)=>{el.onclick=()=>openRun(runs[i]);el.onkeydown=e=>{if(e.key==='Enter')openRun(runs[i])}})}
const RUN_ICON_TOTAL='<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><circle cx="9.5" cy="10" r="2.2"/><circle cx="14.5" cy="10" r="2.2"/><circle cx="12" cy="14.8" r="2.2"/></svg>';
const RUN_ICON_CHECK='<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21.801 10A10 10 0 1 1 17 3.335"/><path d="m9 11 3 3L22 4"/></svg>';
const RUN_ICON_X='<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><path d="m15 9-6 6"/><path d="m9 9 6 6"/></svg>';
...
function renderRunView(target){const run=currentRun;if(!run){target.innerHTML='<div class="empty-action"><h3>No batch selected</h3><p>Go back to Home and pick a batch.</p><button class="primary" data-view="home">Back to Home</button></div>';target.querySelectorAll('[data-view]').forEach(b=>b.addEventListener('click',()=>showView(b.dataset.view)));return}
 const b=runBreakdown(run);
 target.innerHTML=`<div class="patient-detail patient-detail-v2"><div class="case-top-v2"><div class="case-badges"><span class="case-id-badge">${escapeHtml(run.run||'Run')}</span><span class="case-test-badge">${b.total} embryo${b.total===1?'':'s'}</span></div><div class="case-top-actions"><button class="icon-btn" id="closeRun" title="Back to Home" aria-label="Back to Home">${ICON_X}</button></div></div><h2 class="patient-name-v2">${escapeHtml(run.fileName||'')}</h2><div class="patient-meta-v2">${run.by?`Uploaded by ${escapeHtml(run.by)}`:'Uploaded'}${run.at?` · ${new Date(run.at).toLocaleString()}`:''}</div><div class="summary-strip summary-cards"><div class="sum-card tone-teal${b.total?'':' is-zero'}"><span class="sum-icon">${RUN_ICON_TOTAL}</span><strong>${b.total}</strong><small>Total embryos</small></div><div class="sum-card tone-green${b.euploid?'':' is-zero'}"><span class="sum-icon">${RUN_ICON_CHECK}</span><strong>${b.euploid}</strong><small>Euploid</small></div><div class="sum-card tone-red${b.aneuploid?'':' is-zero'}"><span class="sum-icon">${RUN_ICON_X}</span><strong>${b.aneuploid}</strong><small>Aneuploid</small></div><div class="sum-card tone-amber${b.mosaicInc?'':' is-zero'}"><span class="sum-icon">${RUN_ICON_Q}</span><strong>${b.mosaicInc}</strong><small>Mosaic / Inconclusive</small></div><div class="sum-card tone-blue${b.pass?'':' is-zero'}"><span class="sum-icon">${RUN_ICON_CHECK}</span><strong>${b.pass}</strong><small>QC Pass</small></div><div class="sum-card tone-grey${b.fail?'':' is-zero'}"><span class="sum-icon">${RUN_ICON_X}</span><strong>${b.fail}</strong><small>QC Fail</small></div></div><div class="embryo-table-card"><div class="embryo-table-head"><div class="embryo-table-title">${ICON_DNA}<h3>Samples in this batch <span class="count-badge">(${b.total})</span></h3></div></div><div class="embryo-table-scroll"><table class="embryo-table"><thead><tr><th>Patient</th><th>Sample ID</th><th>Embryo</th><th>QC</th><th>Karyotype</th><th>Result</th></tr></thead><tbody>${b.list.map(x=>{const out=conclusionClass(x.data)||'N/A',qcv=qcVerdict(x.data)||'—',kt=field(x.data,['karyotype mwf','karyotype normal wf'])||'—',sampleId=field(x.row,['sample id'])||x.case.id;return `<tr class="embryo-row" data-case="${escapeHtml(x.case.id)}"><td><strong>${escapeHtml(x.case.patient)}</strong></td><td class="mono">${escapeHtml(sampleId)}</td><td>${escapeHtml(x.tag)}</td><td>${escapeHtml(qcv)}</td><td>${escapeHtml(kt)}</td><td><span class="result-pill ${out.toLowerCase().replace(/[^a-z]/g,'')}">${out}</span></td></tr>`}).join('')}</tbody></table></div></div></div>`;
 $('#closeRun').onclick=()=>showView('home');
...
async function renderHomeCards(){await loadResultFiles();renderHomeStats();renderRunList()}
...
function fitClientLegendHeight(){const legend=$('#clientLegend'),card=document.querySelector('.client-dist-card'),sibling=document.querySelector('.quality-card');if(!legend||!card||!sibling)return;const footer=card.querySelector(':scope>.text-button'),footerH=footer?footer.getBoundingClientRect().height+16:0,h=Math.round(sibling.getBoundingClientRect().bottom-legend.getBoundingClientRect().top-footerH);legend.style.maxHeight=`${Math.max(180,h)}px`}
window.addEventListener('resize',()=>fitClientLegendHeight());
...
function showView(view){
 ...
 $$('.nav-item').forEach(n=>n.classList.toggle('active',n.dataset.view===view||(['cases','reportprep','rebiopsy'].includes(view)&&n.dataset.view==='samples')));
