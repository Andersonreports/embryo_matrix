...
function fullRegistryDataset(){const rows=cases.flatMap(c=>c.embryos?.length?c.embryos.map((e,i)=>({case:c,embryo:e,identity:resultIdentity(e),outcome:e._outcome||c.result})):[{case:c,embryo:{},identity:{embryo:'Sample 1'},outcome:c.result}]),cols=embryoColumns(rows);return{headers:cols.map(k=>k.toUpperCase()),rows:rows.map(x=>cols.map(k=>x.embryo[k]??''))}}
