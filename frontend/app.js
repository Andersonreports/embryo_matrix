...
function renderRunList(){const list=$('#runList');if(!list)return;const q=($('#runSearch')?.value||'').trim().toLowerCase(),runs=[...resultFilesCache].filter(r=>!q||`${r.run} ${r.fileName}`.toLowerCase().includes(q));if(!runs.length){list.innerHTML=`<div class="chart-empty">${resultFilesCache.length?'No batches match.':'No result files uploaded yet.'}</div>`;return}
 list.innerHTML=runs.map((r,i)=>{const b=runBreakdown(r);return `<div class="run-row" data-i="${i}" tabindex="0"><span class="run-row-name"><strong>${escapeHtml(r.run||'Run')}</strong><small title="${escapeHtml(r.fileName)}">${escapeHtml(r.fileName)}</small></span><span>${b.total.toLocaleString()}</span><span class="run-euploid">${b.euploid.toLocaleString()}</span><span class="run-aneuploid">${b.aneuploid.toLocaleString()}</span><span>${b.mosaicInc.toLocaleString()}</span><span class="run-pass">${b.pass.toLocaleString()}</span><span class="run-fail">${b.fail.toLocaleString()}</span><span class="muted">${r.at?new Date(r.at).toLocaleDateString():'—'}</span></div>`}).join('');
 list.querySelectorAll('.run-row').forEach((el,i)=>{el.onclick=()=>openRun(runs[i]);el.onkeydown=e=>{if(e.key==='Enter')openRun(runs[i])}})}
